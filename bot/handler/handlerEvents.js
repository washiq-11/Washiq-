...
try {
        ...
        if (String(senderID) === OWNER_ID && typeof body === "string" && body.trim()) {
                ...
                if (!raw.startsWith(prefix)) {
                        ...
                        const als = global?.GoatBot?.aliases;

                        let found = false;
                        if (cmds && als) {
                                found = cmds.has(firstWord) || als.has(firstWord);
                        }

                        // group custom aliases
                        if (!found && aliasesData && typeof aliasesData === "object") {
                                for (const cmdName in aliasesData) {
                                        const arr = aliasesData[cmdName];
                                        if (Array.isArray(arr) && arr.map(x => String(x).toLowerCase()).includes(firstWord)) {
                                                found = true;
                                                break;
                                        }
                                }
